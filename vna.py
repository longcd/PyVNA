...
class VNA:
    ...
    def active_trace(self, tr: int, ch: int = 1) -> None:
        ...
        self._instr.write(":CALC{ch}:PAR{tr}:SEL".format(ch, tr))

    def trace_number(self, num: int, ch: int = 1) -> None:
        """Sets/gets the number of traces of selected channel(Ch)."""
        self._instr.write(":CALC{ch}:PAR:COUN {num}".format(ch, num))
    ...
    def freq_start(self, freq: Union[float, str], ch: int = 1) -> str:
        ...
        if isinstance(freq, str) and freq == "?":
            return self._instr.query(":SENS{ch}:FREQ:STAR?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:FREQ:STAR {freq}".format(ch, freq))
    ...
    def freq_stop(self, freq: Union[float, str], ch: int = 1) -> str:
        ...
        if isinstance(freq, str) and freq == "?":
            ...
            return self._instr.write(":SENS{ch}:FREQ:STOP {freq}".format(ch, freq))
    ...
    def freq_center(self, freq: Union[float, str], ch: int = 1) -> str:
        ...
        """
        if isinstance(freq, str) and freq == "?":
            return self._instr.query(":SENS{ch}:FREQ:CENT?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:FREQ:CENT {freq}".format(ch, freq))
    ...
    def freq_span(self, freq: Union[float, str], ch: int = 1) -> str:
        ...
        """
        if isinstance(freq, str) and freq == "?":
            return self._instr.query(":SENS1:FREQ:SPAN?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:FREQ:SPAN {freq}".format(ch, freq))
    ...
    def power(self, power: Union[float, str], ch: int = 1) -> str:
        ...
        if isinstance(power, str) and power == "?":
            return self._instr.query(":SOUR{ch}:POW?".format(ch))
        else:
            return self._instr.write(":SOUR{ch}:POW {power}".format(ch, power))
    ...
    def points(self, point: Union[int, str], ch: int = 1) -> str:
        ...
        if isinstance(point, str) and point == "?":
            return self._instr.query(":SENS{ch}:SWE:POIN?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:SWE:POIN {point}".format(ch, point))
    ...
    def sweep_type(self, typ: str, ch: int = 1) -> str:
        ...
        """
        if typ == "?":
            return self._instr.query(":SENS{ch}:SWE:TYPE?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:SWE:TYPE {typ}".format(ch, typ))

    def segm_data(self, data: str, ch: int = 1) -> str:
        ...
        """
        if data == "?":
            return self._instr.query(":SENS{ch}:SEGM:DATA?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:SEGM:DATA {data}".format(ch, data))
    ...
    def bandwidth(self, bw: Union[float, str], ch: int = 1) -> str:
        ...
        if isinstance(bw, str) and bw == "?":
            return self._instr.query(":SENS{ch}:BAND?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:BAND {bw}".format(ch, bw))
    ...
    def trace_num(self, num: Union[int, str], ch: int = 1) -> str:
        ...
        if isinstance(num, str) and num == "?":
            return self._instr.query(":CALC{ch}:PAR:COUN?".format(ch))
        else:
            return self._instr.write(":CALC{ch}:PAR:COUN {num}".format(ch, num))
    ...
    def window_layout(self, layout: str, ch: int = 1) -> str:
        ...
        """
        if layout == "?":
            return self._instr.query(":DISP:WIND:SPL?")
        else:
            return self._instr.write(":DISP:WIND:SPL {}".format(layout))
    ...
    def parameter(self, para: str, tr: int, ch: int = 1) -> str:
        ...
        """
        if para == "?":
            return self._instr.query(":CALC{ch}:PAR{tr}:DEF?".format(ch, tr))
        else:
            return self._instr.write(
                ":CALC{ch}:PAR{tr}:DEF {para}".format(ch, tr, para)
            )
    ...
    def format(self, form: str, tr: int, ch: int = 1) -> str:
        ...
        """
        if form == "?":
            return self._instr.query(":CALC{ch}:TRAC{tr}:FORM?".format(ch, tr))
        else:
            return self._instr.write(
                ":CALC{ch}:TRAC{tr}:FORM {form}".format(ch, tr, form)
            )
    ...
    def cal_kit(self, kit: Union[int, str], ch: int = 1) -> str:
        ...
        """
        if kit == "?":
            return self._instr.query(":SENS{ch}:CORR:COLL:CKIT?".format(ch))
        else:
            return self._instr.write(":SENS{ch}:CORR:COLL:CKIT {kit}".format(ch, kit))
    ...
    def cal_meth(self, ports: str, ch: int = 1) -> str:
        ...
        """
        return self._instr.write(
            ":SENS{ch}:CORR:COLL:METH:SOLT2 {ports}".format(ch, ports)
        )
    ...
    def cal_open(self, port: int, ch: int = 1) -> None:
        ...
        port: int
            the specified port
        """
        self._instr.write(":SENS{ch}:CORR:COLL:OPEN {port}".format(ch, port))
    ...
    def cal_shor(self, port: int, ch: int = 1) -> None:
        ...
        port: int
            the specified port
        """
        self._instr.write(":SENS{ch}:CORR:COLL:SHOR {port}".format(ch, port))
        self._instr.query("*OPC?")
    ...
    def cal_load(self, port: int, ch: int = 1) -> None:
        ...
        port: int
            the specified port
        """
        self._instr.write(":SENS{ch}:CORR:COLL:LOAD {port}".format(ch, port))
        self._instr.query("*OPC?")

    def cal(self, typ: str, port: int, ch: int = 1) -> None:
        self._instr.write(":SENS{ch}:CORR:COLL:{typ} {port}".format(ch, typ, port))
        self._instr.query("*OPC?")
    ...
    def cal_thru(self, port1: int, port2: int, ch: int = 1) -> None:
        ...
        self._instr.write(
            ":SENS{ch}:CORR:COLL:THRU {port1},{port2}".format(ch, port1, port2)
        )
        self._instr.write(
            ":SENS{ch}:CORR:COLL:THRU {port2},{port1}".format(ch, port1, port2)
        )
        self._instr.query("*OPC?")

    def cal_done(self, ch: int = 1) -> None:
        self._instr.write(":SENS{ch}:CORR:COLL:SAVE".format(ch))
    ...
    def save_type(self, stype: str) -> str:
        ...
        if stype == "?":
            ...
            return self._instr.write(":MMEM:STOR:STYP {}".format(stype))

    def mdir(self, folder: str) -> None:
        self._instr.write(":MMEM:MDIR '{folder}'".format(folder))
    ...
    def stor_stat(self, file: str) -> None:
        ...
        """
        self._instr.write(":MMEM:STOR '{file}'".format(file))
    ...
    def load_stat(self, file: str) -> None:
        ...
        file: str
            ...
            eg. 'D:/FILTERS/test.sta'
        """
        self._instr.write(":MMEM:LOAD '{file}'".format(file))
    ...
    def init_cont(self, status: str, ch: int = 1) -> None:
        ...
        self._instr.write(":INIT{ch}:CONT {status}".format(ch, status))

    def trigger(self, ch: int = 1) -> None:
        self._instr.write(":INIT{ch}".format(ch))
    ...
    def limit_display(self, state: str, tr: int, ch: int = 1) -> str:
        ...
        """
        if state == "?":
            return self._instr.query(":CALC{ch}:TRAC{tr}:LIM:DISP?".format(ch, tr))
        else:
            return self._instr.write(
                ":CALC{ch}:TRAC{tr}:LIM:DISP {state}".format(ch, tr, state)
            )
    ...
    def limit_data(self, data: str, tr: int, ch: int = 1) -> str:
        ...
        """
        if data == "?":
            return self._instr.query(":CALC{ch}:TRAC{tr}:LIM:DATA?".format(ch, tr))
        else:
            return self._instr.write(
                ":CALC{ch}:TRAC{tr}:LIM:DATA {data}".format(ch, tr, data)
            )
